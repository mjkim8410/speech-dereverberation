...
def build_model(CONTINUE_FROM_CHECKPOINT):
    ...
    if CONTINUE_FROM_CHECKPOINT:
        checkpoint_path = "../../checkpoints_v2/v2_model_epoch_"+str(ITER-1)+".pth"  # CHANGE to your checkpoint
...
def train_one_epoch(model, optimizer, epoch):
    ...
    loop = tqdm(dloader, total=len(dloader), desc=f"Epoch [{epoch+ITER+1}/{NUM_EPOCHS+ITER}]", unit="batch", position=0)
    current_loss = 0.0
    total_loss = 0.0
    total_excess = 0.0
    clip = 0
    ...
    checkpoint_path = os.path.join("../../checkpoints_v2", f"v2_model_epoch_{epoch+ITER+1}.pth")
    torch.save({
        "epoch": epoch+ITER+1,
